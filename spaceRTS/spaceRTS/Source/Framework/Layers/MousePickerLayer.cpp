...
namespace MYENGINE
{
	glm::vec3 WorldLayerBase::CalculateMouseWorldPoint(float x, float y) {
	...
	glm::vec2 WorldLayerBase::GetNormalisedDeviceCoordinates(float mouseX, float mouseY) {
		int viewport[4];
	...
	glm::vec4 WorldLayerBase::ToEyeCoords(glm::vec4 clipCoords) {
	...
	glm::vec3 WorldLayerBase::ToWorldCoords(glm::vec4 eyeCoords) {
	...
	glm::vec3 WorldLayerBase::PointOnXYPlane(glm::vec3 rayFar, glm::vec3 rayNear)
	{
		glm::vec3 planeP = glm::vec3(1.0f,1.0f,0.0f);
